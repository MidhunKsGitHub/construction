...
class FetchAddressIntentServices(name: String?) : IntentService(name) {
    var resultReceiver: ResultReceiver? = null
    override fun onHandleIntent(intent: Intent?) {

        if (intent != null) {
            var errormessgae = ""
            resultReceiver = intent.getParcelableExtra<ResultReceiver>(Constants.RECEVIER)
            ...
            if (addresses == null || addresses.size == 0) {
                ...
                val str_district = address.subLocality
